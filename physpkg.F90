...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
  ...
  subroutine tphysac (ztodt,   cam_in,  &
    ...
    use physconst,          only: pi                    !amip-TIP-nosh
   ...
   ! --------------------amip-TIP-nosh----------------------- !
    do i = 1, ncol
       ...
       if (state%phis(i) > 4900) then

          if (state%lat(i) >= 15/180._r8*pi .and. state%lat(i) <= 75/180._r8*pi .and. state%lon(i) >= 70/180._r8*pi .and. state%lon(i) <= 180/180._r8*pi) then
             cam_in%shf(i) = cam_in%shf(i)*0.5_r8
          end if
          if (state%lat(i) >= 20/180._r8*pi .and. state%lat(i) <= 75/180._r8*pi .and. state%lon(i) >= 50/180._r8*pi .and. state%lon(i) <= 70/180._r8*pi) then
             cam_in%shf(i) = cam_in%shf(i)*0.5_r8
          end if
          if (state%lat(i) >= 35/180._r8*pi .and. state%lat(i) <= 45/180._r8*pi .and. state%lon(i) >= 25/180._r8*pi .and. state%lon(i) <= 50/180._r8*pi) then
             cam_in%shf(i) = cam_in%shf(i)*0.5_r8
          end if
          if (state%lat(i) >= 30/180._r8*pi .and. state%lat(i) <= 35/180._r8*pi .and. state%lon(i) >= 45/180._r8*pi .and. state%lon(i) <= 50/180._r8*pi) then
             cam_in%shf(i) = cam_in%shf(i)*0.5_r8
          end if
          if (state%lat(i) >= 5/180._r8*pi .and. state%lat(i) <= 15/180._r8*pi .and. state%lon(i) >= 70/180._r8*pi .and. state%lon(i) <= 90/180._r8*pi) then
             cam_in%shf(i) = cam_in%shf(i)*0.5_r8
          end if
